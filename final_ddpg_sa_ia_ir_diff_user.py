        ...
        for i_episode in range(episode):
            ...
            for i_iteration in range(max_iteration):
                ...
                if i_iteration == max_iteration-1:
                    ...
                    print('i_seed =',i_seed,'numUser =', numAPuser, 'i_episode =',i_episode, 'reward =',reward_ave, 'system_bitrate =',system_bitrate_ave)

            if (i_episode+1) % 50 == 0 or i_episode == 0:
                dataframe=pd.DataFrame({'bitrate':actor_loss_history})
                dataframe.to_csv("E:/FYP/Modification Code/DDPG_OFDMA_interference/result/ddpg_actor_loss_sa_ia_ir_seed_"+str(i_seed)+"_numUser_"+str(numAPuser)+"_episode_"+str(i_episode)+".csv", index=False,sep=',')
                dataframe=pd.DataFrame({'bitrate':critic_loss_history})
                dataframe.to_csv("E:/FYP/Modification Code/DDPG_OFDMA_interference/result/ddpg_critic_loss_sa_ia_ir_seed_"+str(i_seed)+"_numAPuser_"+str(numAPuser)+"_episode_"+str(i_episode)+".csv", index=False,sep=',')
 
        dataframe=pd.DataFrame({'bitrate':system_ave_bitrate_history})
        dataframe.to_csv("E:/FYP/Modification Code/DDPG_OFDMA_interference/result/ddpg_bitrate_sa_ia_ir_seed_"+str(i_seed)+"_numAPuser_"+str(numAPuser)+".csv", index=False,sep=',')
        dataframe=pd.DataFrame({'reward':reward_ave_history})
        dataframe.to_csv("E:/FYP/Modification Code/DDPG_OFDMA_interference/result/ddpg_reward_sa_ia_ir_seed_"+str(i_seed)+"_numAPuser_"+str(numAPuser)+".csv", index=False,sep=',')